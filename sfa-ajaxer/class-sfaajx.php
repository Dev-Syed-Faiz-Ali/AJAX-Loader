...
class sfaAjax {
    ...
    function __construct() {
        ...
        add_action('wp_ajax_sfaajx_ajax_mo', array($this, 'post_save_mo'));
        add_action('wp_ajax_sfaajx_dismiss_notice', array($this, 'post_save_notice'));
    ...
    function handle_init() {
        ...
        if (is_admin()) {
            wp_enqueue_script('sfaajx-admin-global', $this->base_url . 'admin/admin-global.js');
            wp_enqueue_style('sfaajx-admin-global', $this->base_url . 'admin/admin-global.css');
            if (isset($_GET['page']) && $_GET['page'] == $this->adminpagename_mainoptions) {

                if(isset($_GET['sfaajx_shortcode_builder']) && $_GET['sfaajx_shortcode_builder']=='on'){
...
//                    wp_enqueue_script('admin-mo', $this->base_url . 'assets/sfacheckbox/admin-mo.js');
                    wp_enqueue_style('sfacheckbox', $this->base_url . 'assets/sfacheckbox/sfacheckbox.css');
                    wp_enqueue_script('sfaselector', $this->base_url . 'assets/sfaselector/sfaselector.js');
                    wp_enqueue_style('sfaselector', $this->base_url . 'assets/sfaselector/sfaselector.css');
                }
            }
            ...
            wp_enqueue_script('sfaajx', $this->base_url . 'assets/ajaxer.js', array('jquery'),$ver);
            wp_enqueue_style('sfaajx', $this->base_url . 'assets/ajaxer.css');


            if($this->is_preview){

                wp_enqueue_script('sfaajx.preseter', $this->base_url . 'assets/preseter/preseter.js');
                wp_enqueue_style('sfaajx.preseter', $this->base_url . 'assets/preseter/preseter.css');
            }
        }
    }
    ...
    function handle_admin_notices(){
        ...
        if($this->db_mainoptions['content_container_selector']=='') {
            if ((get_option('sfaajx-notice-dismissed'))=='') {
                ?>

                <div class="notice  sfaajx-notice is-dismissible">
                    <p><?php printf(__('sfa Ajaxer needs configuration. Set it up %1$shere%2$s.'), '<a href="' . admin_url("admin.php?page=" . $this->adminpagename_mainoptions) . '">', '</a>'); ?></p>
                </div>
                <?php
            }
        }
    ...
    function post_save_notice() {
        ...
        update_option('sfaajx-notice-dismissed', 'on');
        die();
    }
...
//            console.info(window.preseter_init);
                ...
                window.preseter_init()
                            ...
                            if(sfaajx_get_query_arg(window.location.href,'transition')){

//                                console.info(sfaajx_get_query_arg(window.location.href,'transition'))
                                $('*[name="transition"]').val(sfaajx_get_query_arg(window.location.href,'transition'));
                            }
                            if(sfaajx_get_query_arg(window.location.href,'preloader')){

//                                console.info(sfaajx_get_query_arg(window.location.href,'transition'))
                                $('*[name="preloader"]').val(sfaajx_get_query_arg(window.location.href,'preloader'));
                            }
        ...
        <form class="preseter align-right wait-for-activate preseter-opened-by-user" style="">
            ...
            <div class="preseter-content-con auto-height overflow-x-visible" style="width: 160px; height: auto;"> <div class=" the-content-inner-con"> <div class="the-content inner" style=" " data-targetw="-160"> <div class="the-content-inner-inner">
                            ...
                            <div class="setting">
                                ...
                                $lab = "transition";
                                echo sfaHelpers::generate_select($lab,array(
                                    "options"=>$arr_opts,
...
?><script>
    ...
    function handle_admin_menu() {
        ...
        $sfaajx_page = add_menu_page(__('Ajaxer', 'sfaajx'), __('SFA Settings', 'sfaajx'), $this->admin_capability, $this->adminpagename_mainoptions, array($this, 'admin_page_mainoptions'), 'div');
    ...
    function admin_page_mainoptions() {
        ...
        if (isset($_POST['sfaajx_delete_alloptions']) && $_POST['sfaajx_delete_alloptions'] == 'on') {
            //delete_option('sfaajx_items');
            delete_option($this->dboptionsname);
            //delete_option('sfaajx_biggalleries');
        }
...
//        echo 'the theme - '.get_stylesheet();
        ...
        if(isset($_GET['sfaajx_shortcode_builder']) && $_GET['sfaajx_shortcode_builder']=='on'){
            sfaajx_shortcode_builder();
        }else {

            ?>

            <div class="wrap">
                <h2><?php echo __('Main Settings', 'sfaajx'); ?></h2>
                ...
                <form class="mainsettings">
                    ...
                    <h1><?php echo __('OPTIONS', 'sfaajx'); ?></h1>
                    <div class="setting">

                        <?php
                        $lab = 'enable_ajax';
                        echo sfaHelpers::generate_input_text($lab,array('id' => $lab, 'val' => 'off','input_type'=>'hidden'));
                        ?>
                        <h4 class="setting-label"><?php echo __('Enable Plugin Setting?','sfaapp'); ?></h4>
                        <div class="sfacheckbox skin-nova">
                            <?php
                            echo sfaHelpers::generate_input_checkbox($lab,array('id' => $lab, 'val' => 'on','seekval' => $this->db_mainoptions[$lab])); ?>
                            <label for="<?php echo $lab; ?>"></label>
                        </div>
                        <div class="sidenote"><?php echo __('enable the plugin','sfaajx'); ?></div>
                    </div>
 <div class="box">
                    <?php $lab = 'content_container_selector'; ?>
                    <div class="setting">
                        <h4 class="label"><?php echo __('Content Selector', 'sfaajx'); ?></h4>
                        <?php echo sfaHelpers::generate_input_text($lab, array('class' => 'textfield', 'seekval' => $this->db_mainoptions[$lab])); ?>
                        <div class="sidenote"><?php echo __('this is the content identifier, this content will be updated on each page change ', 'sfaajx'); ?></div>
                    </div>

                    <?php $lab = 'menu_selector'; ?>
                    <div class="setting">
                        <h4 class="label"><?php echo __('Menu Selector', 'sfaajx'); ?></h4>
                        <?php echo sfaHelpers::generate_input_text($lab, array('class' => 'textfield', 'seekval' => $this->db_mainoptions[$lab])); ?>
                        <div class="sidenote"><?php echo __('this is the menu identifier, define this in order for the menu to highlight and select the current menu item ', 'sfaajx'); ?></div>
                    </div>
                    </div>



        <div class="box1 version-pro">                 
<div class="mainsl">
...
<div class="htmlbox version-pro">
                    <?php $lab = 'custom_preloader_html'; ?>
                    <div class="setting">
                        <h4 class="label"><?php echo __('Custom Preloader html', 'sfaajx'); ?></h4>
                        <?php echo sfaHelpers::generate_input_textarea($lab, array('class' => 'textfield', 'extraattr' => ' style="resize:both;"', 'seekval' => stripslashes($this->db_mainoptions[$lab]))); ?>
                        <div class="sidenote"><?php echo __('only if preloader is set to custom - this html will take place', 'sfaajx'); ?></div>
                    </div>
                    </div>
                    </div>

                    <div class="box3 version-pro">
                    <div class="setting">
                        ...
                        echo sfaHelpers::generate_input_text($lab,array('id' => $lab, 'val' => 'off','input_type'=>'hidden'));
                        ?>
                        <h4 class="setting-label"><?php echo __('Cache Pages ? ','sfaapp'); ?></h4>
                        <div class="sfacheckbox skin-nova">
                            <?php
                            echo sfaHelpers::generate_input_checkbox($lab,array('id' => $lab, 'val' => 'on','seekval' => $this->db_mainoptions[$lab])); ?>
                            <label for="<?php echo $lab; ?>"></label>
                        </div>
                        <div class="sidenote"><?php echo __('cache the pages into memory','sfaajx'); ?></div>
                    </div>
                    ...
                    <div class="setting">
                        ...
                        echo sfaHelpers::generate_input_text($lab,array('id' => $lab, 'val' => 'off','input_type'=>'hidden'));
                        ?>
                        <h4 class="setting-label"><?php echo __('Update Body Class','sfaapp'); ?></h4>
                        <div class="sfacheckbox skin-nova">
                            <?php
                            echo sfaHelpers::generate_input_checkbox($lab,array('id' => $lab, 'val' => 'on','seekval' => $this->db_mainoptions[$lab])); ?>
                            <label for="<?php echo $lab; ?>"></label>
                        </div>
                        <div class="sidenote"><?php echo __('update the body class with the one from the new page','sfaajx'); ?></div>
                    </div>
                        </div>
...
<div class="version-pro">
                    ...
                    <div class="setting">
                        <h4 class="label"><?php echo __('Comment Form Selector', 'sfaajx'); ?></h4>
                        <?php echo sfaHelpers::generate_input_text($lab, array('class' => 'textfield', 'seekval' => $this->db_mainoptions[$lab])); ?>
                        <div class="sidenote"><?php echo __('', 'sfaajx'); ?></div>
                    </div>
                    ...
                    <div class="setting">
                        <h4 class="label"><?php echo __('Search Form Selector', 'sfaajx'); ?></h4>
                        <?php echo sfaHelpers::generate_input_text($lab, array('class' => 'textfield', 'seekval' => $this->db_mainoptions[$lab])); ?>
                        <div class="sidenote"><?php echo __('', 'sfaajx'); ?></div>
                    </div>
                    </div> 
                    <br/>
                    <a href='#'
                       class="button-primary save-btn save-mainoptions"><?php echo __('Save Options', 'sfaajx'); ?></a>

                       <h5 class="setting-label"><?php echo __('Powered By Syed Faiz Ali','sfaapp'); ?></h5>
                </form>
                <br/><br/>
...
//        wp_enqueue_script('sfaajx_configreceiver', $this->thepath . 'tinymce/receiver.js');
