...
if (!function_exists('sfa_savemeta')) {

    function sfa_savemeta($id, $arg2, $arg3 = '') {
...
if (!function_exists('sfa_checked')) {

    function sfa_checked($arg1, $arg2, $arg3 = 'checked', $echo = true) {
        $func_output = '';
...
if (!function_exists('sfa_find_string')) {

    function sfa_find_string($arg, $arg2) {
        $pos = strpos($arg, $arg2);
...
if (!function_exists('sfa_get_excerpt')) {

    //echo 'sfa_get_excerpt 
    //version 1.2';
    function sfa_get_excerpt($pid = 0, $pargs = array()) {
        //print_r($pargs);
        global $post;
        $fout = '';
        $excerpt = '';
...
if (!function_exists('sfa_print_menu')) {

    function sfa_print_menu() {
...
if (!function_exists('sfa_post_date')) {

    function sfa_post_date($pid) {
        $po = get_post($pid);
        //print_r($po);
        if ($po) {
...
if (!function_exists('sfa_pagination')) {

    function sfa_pagination($pages = '', $range = 2) {
        global $paged;
        $fout = '';
        $showitems = ($range * 2) + 1;
...
if (!function_exists('sfa_curr_url')) {

    function sfa_curr_url($pargs=array()) {
...
if (!function_exists('sfa_addAttr')) {

    function sfa_addAttr($arg1, $arg2) {
        $fout = '';
...
if(!function_exists('sfa_addSwfAttr')){
    function sfa_addSwfAttr($arg1, $arg2, $first=false) {
        $fout='';
        //$arg2 = str_replace('\\', '', $arg2);
...
if (!function_exists('sfa_clean')) {

    function sfa_clean($var) {
            ...
            if ($po->post_excerpt != '' && $margs['forceexcerpt'] == false) {
                ...
                if ($margs['readmore_markup'] != '') {
                    ...
                    if (function_exists('continue_reading_link')) {
                        ...
                        if (function_exists('sfa_excerpt_read_more')) {
                            $fout = str_replace('{readmore}', sfa_excerpt_read_more($pid), $fout);
                        } else {
...
jQuery(".with_colorpicker").each(function(){
        ...
        _t.bind("change", function(){
            //console.log(_t);
            jQuery("#customstyle_body").html("body{ background-color:" + $("input[name=color_bg]").val() + "} .sfaportfolio, .sfaportfolio a{ color:" + $("input[name=color_main]").val() + "} .sfaportfolio .portitem:hover .the-title, .sfaportfolio .selector-con .categories .a-category.active { color:" + $("input[name=color_high]").val() + " }");
        });
        _t.trigger("change");
        ...
        static function generate_select($argname, $pargopts) {
            //-- sfaHelpers::generate_select('label', array('options' => array('peritem','off', 'on'), 'class' => 'styleme', 'seekval' => $this->mainoptions[$lab]));
...
//                print_r($matches);
        ...
        static function add_query_arg()
        {
            ...
            sfaHelpers::wp_parse_str($query, $qs);
            $qs = sfaHelpers::urlencode_deep($qs); // this re-URL-encodes things that were already in the query string
            if (is_array($args[0])) {
                $kayvees = $args[0];
        ...
        static function remove_query_arg($key, $query = false)
        {
            if (is_array($key)) { // removing multiple keys
                foreach ($key as $k)
                    $query = sfaHelpers::add_query_arg($k, false, $query);
                return $query;
            }
            return sfaHelpers::add_query_arg($key, false, $query);
        }
        ...
        static function stripslashes_deep($value)
        {
            if (is_array($value)) {
                ...
                foreach ($vars as $key => $data) {
                    $value->{$key} = sfaHelpers::stripslashes_deep($data);
                }
        ...
        static function wp_parse_str($string, &$array)
        {
            ...
            if (function_exists('stripslashes_deep')) {
                if (get_magic_quotes_gpc()) {
                    $array = sfaHelpers::stripslashes_deep($array);
                }
            }
             ...
             */
            $array = sfaHelpers::apply_filters('wp_parse_str', $array);
        }
...
//                _wp_call_all_hook($args);
        ...
        {
            $value = is_array($value) ? array_map('sfaHelpers::urlencode_deep', $value) : urlencode($value);
            return $value;
        }

        static function build_query($data)
        {
            return sfaHelpers::_http_build_query($data, null, '&', '', false);
        }
        ...
        static function _http_build_query($data, $prefix = null, $sep = null, $key = '', $urlencode = true)
        {
            ...
            foreach ((array)$data as $k => $v) {
                ...
                if (is_array($v) || is_object($v))
                    array_push($ret, sfaHelpers::_http_build_query($v, '', $sep, $k, $urlencode));
                elseif ($urlencode)
        ...
        static function transform_to_str_size($arg) {
            //-- sfaHelpers::transform_to_str_size(400%);
            $fout = $arg;
