...
function sfaajx_mo_ready($){
    ...
    if(window.sfaajx_theme_settings){
        var _t =jQuery('.install-default-settings-for-theme');

        _t.html(_t.attr('data-starttext')+ window.sfaajx_theme_settings.theme_nicename);
    ...
    function handle_mouse(e){
        ...
        if(e.type=='click'){
            if(_t.hasClass('install-default-settings-for-theme')){
                ...
                if(window.sfaajx_theme_settings){
                    for(var lab in window.sfaajx_theme_settings){
                        console.info(lab);

                        var val =  window.sfaajx_theme_settings[lab];
...
function mo_saveall(){
    ...
    var data = {
        action: 'sfaajx_ajax_mo',
        postdata: mainarray
    };
    jQuery('.saveconfirmer').html('successfully updated');
