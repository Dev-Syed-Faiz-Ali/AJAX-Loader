...
//        console.info(this, o);
        ...
        this.each(function () {
            ...
            _t_class = _t_class.replace('sfa-style-me', '');
            ...
            if(_t.next().hasClass('sfa-style-me-feeder')){
                _feeder = _t.next();
            }
            //console.info(_t);

            if(_t.hasClass('treated') || _t.parent().hasClass('sfa-select-wrapper')){
                ...
                if(_t.hasClass('skin-justvisible')){
                    ...
                    if(_wrapper){
                        _wrapper.find('.sfa-select-wrapper-head').unbind('click');
                        _wrapper.find('.sfa-select-wrapper-head').bind('click', function(){
            ...
            _t.wrap('<span class="sfa-select-wrapper '+_t_class+'" style="'+str_w+'"></span>');

            _wrapper = _t.parent();

            _wrapper.prepend('<span class="sfa-select-wrapper-head"><span class="the-text">'+_t.val()+'</span></span>');
            ...
            _theSelect.bind('change', function(){
                ...
                //console.info(_t.parent().find('.sfa-select-wrapper-head .the-text'));
                _t.parent().find('.sfa-select-wrapper-head .the-text').html($(this).find(':selected').html());
            })
            ...
            if(String(_t_class).indexOf('opener-')>-1){
                _t.parent().bind('click',function(){
                    ...
                    setTimeout(function(){
                        ...
                        //console.log( _t.parent().find('.sfalayouter .items-con'));
                        if($.fn.masonry){

                            _t.parent().find('.sfalayouter .items-con').masonry(auxoptions);
                        }
                    },500)
    ...
    window.sfasel_init = function(selector, settings) {
        if(typeof(settings)!="undefined" && typeof(settings.init_each)!="undefined" && settings.init_each==true ){
            ...
            $(selector).each(function(){
                var _t = $(this);
                _t.sfaselector(settings)
            });
        }else{
            $(selector).sfaselector(settings);
        }
...
jQuery(document).ready(function($){
    ...
    //console.info($('select.sfa-style-me'));
    sfasel_init('select.sfa-style-me', {init_each: true});

    jQuery(".version-pro input"). attr('disabled','disabled');
    jQuery(".version-pro select"). attr('disabled','disabled');
    jQuery(".version-pro textarea"). attr('disabled','disabled');
});
