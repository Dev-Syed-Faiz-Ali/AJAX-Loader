...
function sfaajx_get_query_arg(purl, key){
	//console.info(purl);
...
// 	};
	...
	// console.info(jQuery('body').hasClass('sfaajx-enable-reinit'));
...
jQuery(document).ready(function($){
	...
	var _theActualNav = null
		...
		,_sfaajx_styles_con = null
		,_sfaajx_scripts_tbe_con = null
		,_sfaajx_caches_con = null
		,_preloader = null
	...
	if(window.sfaajx_settings) {


		if (sfaajx_settings.site_url) {

			ajax_site_url = sfaajx_settings.site_url;
		}
		if (sfaajx_settings.content_container_selector) {

			_content = $(sfaajx_settings.content_container_selector).eq(0);
			content_selector = sfaajx_settings.content_container_selector;
		}
		if (sfaajx_settings.menu_selector) {

			_theActualNav = $(sfaajx_settings.menu_selector);


			if(_content.find(sfaajx_settings.menu_selector).length>0){
				refresh_menu=true;
			}


			if (sfaajx_settings.menu_move_on_top_of_content && sfaajx_settings.menu_move_on_top_of_content=='on') {
		...
		if (sfaajx_settings.transition) {


			_html_for_classes.addClass('sfaajx-transition-'+sfaajx_settings.transition);
			_html_for_classes.addClass('sfaajx-preloader-'+sfaajx_settings.preloader);
		}
		if (sfaajx_settings.scripts_execute_after_ajax_call) {


			scripts_execute_after_ajax_call = sfaajx_settings.scripts_execute_after_ajax_call;
		}




		if(sfaajx_settings.script_call_on_ready){
			try{
				eval(sfaajx_settings.script_call_on_ready)
			}catch(err){
	...
	_content.addClass('sfaajx-content');
	// console.info(_content);

	if(window.sfaajx_settings) {
		if (sfaajx_settings.preloader=='bar' || sfaajx_settings.preloader=='bars' || sfaajx_settings.preloader=='custom') {

			var aux ='<div class="sfaajx-preloader sfaajx-preloader-'+sfaajx_settings.preloader+'">';

			if(sfaajx_settings.preloader=='bar'){
				aux+='<div class="the-bar"></div>'
			}
			if(sfaajx_settings.preloader=='bars'){
			...
			if(sfaajx_settings.preloader=='custom'){
				aux+='<div id="loading-center-absolute"> '+sfaajx_settings.custom_preloader_html+' </div>'
			}


			aux+='</div>';
			if(_content.prev().hasClass('sfaajx-preloader')==false){

				_content.before(aux);
			}
			_preloader = $('.sfaajx-preloader').eq(0);
		}
	}
	...
	_body.append('<div class="sfaajx-styles-con"></div>');

	_sfaajx_styles_con = _body.children('.sfaajx-styles-con');

	if(scripts_execute_after_ajax_call=='on'){

		_body.append('<div class="sfaajx-scripts-con"></div>');

		_sfaajx_scripts_tbe_con = _body.children('.sfaajx-scripts-con');
	}

	if(sfaajx_settings.cache_pages=='on'){

		_body.append('<div class="sfaajx-caches-con"></div>');

		_sfaajx_caches_con = _body.children('.sfaajx-caches-con');
	}
	...
	if(sfaajx_settings.classes_to_ignore){

		selecta+=','+sfaajx_settings.classes_to_ignore;
	}
	...
	if(sfaajx_settings.comment_form_selector){
		$(document).delegate(sfaajx_settings.comment_form_selector, 'submit',handle_submit);
	}
	if(sfaajx_settings.search_form_selector){
		$(document).delegate(sfaajx_settings.search_form_selector, 'submit',handle_submit_search);
	}


	window.sfaajx_click_menu_anchor = click_menu_anchor;
	...
	function handle_popstate(e){
		...
		if(e.state && e.state.href){
			...
			if(sfaajx_settings.use_ajax_on_back_button=='on'){
	...
	function convert_to_html(arg, pargs){
		...
		if(margs.convert_script_tags=='on'){
			...
			// console.warn(fout);
			fout = String(fout).replace(/<script(.*?)>([\s|\S]*?)<\/script>/igm, '<!--start sfaajx-script-tag--><div class="sfaajx-script-tag" $1>$2</div><!--end sfaajx-script-tag-->');
		...
		if(e.type=='submit'){
			...
			var req = $.ajax({
				...
				success: function (data,ts) {
					...
					if(sfaajx_settings.comment_list_selector && curr_html.indexOf('replytocom')==-1  && $(sfaajx_settings.comment_list_selector).eq(0).length>0){




						if(__response.find(sfaajx_settings.comment_list_selector).eq(0).length>0){
							$(sfaajx_settings.comment_list_selector).append(__response.find(sfaajx_settings.comment_list_selector).eq(0).children().last());

							$(sfaajx_settings.comment_list_selector).children().last().css('opacity','0');
							$(sfaajx_settings.comment_list_selector).children().last().animate(
								{'opacity':'1'}
								,{queue:false
									,complete: function(e){
						...
						console.info(__response, __response.find(sfaajx_settings.comment_list_selector).eq(0));
					}else{
						click_menu_anchor(null, {
					...
					$('.comment-feedbacker').html(sfaajx_settings.translate_comment_submitted);
					$('.comment-feedbacker').fadeIn("fast");
	...
	function loadScript(src, callback){
		...
		if(_sfaajx_scripts_tbe_con){
			_sfaajx_scripts_tbe_con.get(0).appendChild(s);
		}else{
	...
	function click_menu_anchor(e,pargs) {
		...
		function onajaxcompleted(response,textStatus,jqXHR){
			...
			if(sfaajx_settings.update_body_class=='on'){
			...
			if(sfaajx_settings.scripts_to_reload){



				var aux_split = String(sfaajx_settings.scripts_to_reload).split(',');
			...
			// console.info( response_html, $(response_html), $(response_html).find('.sfaajx-script-tag'));
			...
			if(scripts_execute_after_ajax_call=='on') {
				var regex_script_tag = /<!--start sfaajx-script-tag--><div class="sfaajx-script-tag"(.*?)>([\s|\S]*?)<\/div><!--end sfaajx-script-tag-->/igm;
				...
				response_html = String(response_html).replace(/<!--start sfaajx-script-tag--><div class="sfaajx-script-tag">([\s|\S]*?)<\/div><!--end sfaajx-script-tag-->/igm, '');
			...
			// $(response_html).find('.sfaajx-script-tag').each(function(){
			//
			// 	var _t24 = $(this);
			//
			...
			if (sfaajx_settings.menu_selector) {
				if(sfaajx_settings.menu_move_on_top_of_content=='on'){

					___response.find(sfaajx_settings.menu_selector).eq(0).remove();
				}
			}
			...
			if(sfaajx_settings.extra_items_to_be_recovered){
				___response.find(sfaajx_settings.extra_items_to_be_recovered).each(function(){
			...
			if(sfaajx_settings.extra_items_to_be_added){

				// console.info(sfaajx_settings.extra_items_to_be_added, ___response.find(sfaajx_settings.extra_items_to_be_added), ___response.find(sfaajx_settings.extra_items_to_be_added).eq(0).html());

				var aux_split = String(sfaajx_settings.extra_items_to_be_added).split(',');
			...
			setTimeout(function () {
				...
				if(_contentContainer){
					...
					// 		_t.append('<div class="col-md-3"></div>');
                    //
					// 		_t.children('.edd_download:nth-child(4n+1)').attr('sfaaux','1');
					// 		_t.children('.edd_download:nth-child(4n+2)').attr('sfaaux','2');
					// 		_t.children('.edd_download:nth-child(4n+3)').attr('sfaaux','3');
					// 		_t.children('.edd_download:nth-child(4n+4)').attr('sfaaux','4');
                    //
					// 		_t.children('.col-md-3').eq(0).append(_t.children('.edd_download[sfaaux=1]'));
					// 		_t.children('.col-md-3').eq(1).append(_t.children('.edd_download[sfaaux=2]'));
					// 		_t.children('.col-md-3').eq(2).append(_t.children('.edd_download[sfaaux=3]'));
					// 		_t.children('.col-md-3').eq(3).append(_t.children('.edd_download[sfaaux=4]'));
                    //
					// 		_t.children('.edd_download').remove();
					// 	}
                    //
				...
				setTimeout(function () {
					//console.info(window.sfaprx_init);
...
//                            console.info(___response, ___response_scriptmo);
		...
		if (window.sfaajx_settings.disable_ajax_on_touch_devices && window.sfaajx_settings.disable_ajax_on_touch_devices=='on' && is_touch_device()) {
...
//        console.info(_t);
		...
		//console.info(window.sfaap_settings.enable_ajax == 'on', window, margs.force_no_ajax)
		if (window.sfaajx_settings.enable_ajax == 'on' && window && margs.force_no_ajax != 'on') {
				...
				if (window.location.href.indexOf('file://') == 0 || ( (thehref.indexOf('http://') > -1||thehref.indexOf('https://') > -1) && thehref.indexOf(ajax_site_url) != 0) || (thehref.indexOf('admin.php')>-1) || (thehref.indexOf('wp-admin')>-1) )   {
					...
					if (can_history_api()) {
						...
						if (_t.hasClass('ajax-link-user')) {
							...
							_html_for_classes.addClass('sfaajx-ajax-transitioning-out ');
							_html_for_classes.addClass('sfaajx-ajax-loading ');
							...
							if (sfaajx_settings.transition=='scaledown'  || sfaajx_settings.transition=='slide') {
								_content.wrap('<div id="sfaajx-content-container"></div>');
						...
						var sw = false;
						if(_sfaajx_caches_con){

							// for(var i25 in pages_caches){
							_sfaajx_caches_con.children().each(function(){
	...
	function ajax_load_new_page(){
		...
		var i23 = 0;
		if(sfaajx_settings.extra_items_to_be_recovered){
			$(sfaajx_settings.extra_items_to_be_recovered).each(function(){
			...
			for ( var i233 in aux_split){
				...
				if(sw){
					...
					if($(_c.selector).length>0){
						...
						if(sfaajx_settings.extra_items_to_be_added_before_elements){

							$(sfaajx_settings.extra_items_to_be_added_before_elements).before(_c.outer_html);
		...
		_sfaajx_styles_con.children().remove();


		for(var i3 = 0; i3<stylesheets_tobeadded.length;i3++){
			_sfaajx_styles_con.append(stylesheets_tobeadded[i3]);
		}


		// -- here we add new page html
		if(_html_for_classes.hasClass('sfaajx-ajax-user-transitioning-out')){
			...
			if(_contentContainer){
				...
				if(_sfaajx_caches_con){
					...
					_content.parent().find('.sfaajx-content.last-content:not(".in-cache")').attr('the_href',old_href);
					_content.parent().find('.sfaajx-content.last-content:not(".in-cache")').attr('class_body',old_class_body);
					...
					_sfaajx_caches_con.children().each(function() {
						...
						// 	_t23 = $('.sfaajx-content.last-content');
						// 	sw = true;
						// 	// break;
						// }
						// }
					...
					if(sw==false){
						...
						console.info('last content - ', $('.sfaajx-content.last-content'));
						_sfaajx_caches_con.append($('.sfaajx-content.last-content'));
						_sfaajx_caches_con.children().addClass('in-cache');
						_content.parent().find('.sfaajx-content.last-content:not(".in-cache")').remove()
			...
			setTimeout(function(){
				...
				_html_for_classes.addClass('sfaajx-ajax-transitioning-in');
			},50)
		...
		_html_for_classes.removeClass('sfaajx-ajax-loading ');
		setTimeout(function(){

			_html_for_classes.removeClass('sfaajx-ajax-transitioning-out');
		},500);
		...
		setTimeout(function(){
			...
			if(sfaajx_settings.scripts_reinit_document_ready=='on'){
			...
			if(sfaajx_settings.script_call_on_reinit){
				try{
					eval(sfaajx_settings.script_call_on_reinit)
				}catch(err){
					console.warn(err);
			...
			// return;
			_html_for_classes.removeClass('sfaajx-ajax-transitioning-in');
			_html_for_classes.removeClass('sfaajx-ajax-user-transitioning-out');
			...
			for(var i in scripts_tobeexecuted){
				...
				if(_t23){
					...
					try{
						...
						if(sfaajx_settings.remove_document_ready && sfaajx_settings.remove_document_ready=='on'){
							// aux = aux.replace()
						...
						try{
							_sfaajx_scripts_tbe_con.get(0).appendChild(sn);
						}catch(err){
